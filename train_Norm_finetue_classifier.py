...
parser.add_argument('--model', type=str, default='dgcnn', choices=['pointnet', 'dgcnn', 'pointgrid'], help='Model to use')
...
parser.add_argument('--Dec_on_src', type=str2bool, default=True, help='Using Decoder in source')
parser.add_argument('--Dec_on_trgt', type=str2bool, default=True, help='Using Decoder in target')
...
parser.add_argument('--cls_weight', type=float, default=0.05, help='weight of the classification loss')
...
opt_cls = optim.SGD(model.cls_C.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.wd) if args.optimizer == "SGD" \
    else optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.wd)
scheduler = CosineAnnealingLR(opt, args.epochs - 10)
scheduler_cls = CosineAnnealingLR(opt, 10)
...
def test(test_loader, model=None, set_type="Target", partition="Val", epoch=0):
    ...
    with torch.no_grad():
        ...
        for data, labels, _ in test_loader:
            ...
            data_grid = data_grid.permute(0, 4, 1, 2, 3)
    ...
    if epoch <= 10:
        for data1, data2 in zip(src_train_loader, trgt_train_loader):
            ...
            if data1 is not None:
                ...
                src_logits = model(src_data, activate_DefRec=False)
                loss = args.cls_weight * criterion_ls(src_logits["cls"], src_label)
        ...
        for data1, data2 in zip(src_train_loader, trgt_train_loader):
            ...
            if data1 is not None:
                ...
                # src_data, src_norm_curv = pc_utils_Norm.remove_region_points(src_data, src_norm_curv, device)
                # change to [batch_size, num_coordinates, num_points]
                src_fea_pc = pc_utils_Norm.extract_feature_points(src_data, src_norm_curv, 512).to(
                    device)  # [B, 512, 3]
                src_data = src_data.permute(0, 2, 1)
                batch_size = src_data.size()[0]
                src_domain_label = torch.zeros(batch_size).long().to(device)
                ...
                # predict with undistorted shape
                src_logits = model(src_data, activate_DefRec=False)
                loss = args.cls_weight * criterion_ls(src_logits["cls"], src_label) * 10
                src_print_losses['cls'] += loss.item() * batch_size
                src_print_losses['total'] += loss.item() * batch_size
                loss.backward()
                src_count += batch_size
